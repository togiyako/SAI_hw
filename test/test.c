...
int main()
{
    ...
    sai_object_id_t port1 = 0x0100000001UL;
    sai_object_id_t port2 = 0x0100000002UL;
    sai_object_id_t port3 = 0x0100000003UL;
    ...
    assert(objlist_contains(port_list, attr.value.objlist.count, port1));
    assert(objlist_contains(port_list, attr.value.objlist.count, port2));
    ...
    assert(objlist_contains(port_list, attr.value.objlist.count, port3));
    ...
    for (int i = 0; i < 16; i++) {
        sai_object_id_t fake_port = 0x0100000010UL + i;
        status = create_lag_member_full(lag_api, &members[i], test_lag, fake_port);
        assert(status == SAI_STATUS_SUCCESS);
    }
    printf("created 16 members\n");

    status = create_lag_member_full(lag_api, &members[16], test_lag, 0x0100000020UL);
    assert(status != SAI_STATUS_SUCCESS);
    ...
    for (int i = 0; i < 15; i++) {
        sai_object_id_t fake_port = 0x0200000010UL + i;
        status = create_lag_member_full(lag_api, &global_members[i], lag_a, fake_port);
        assert(status == SAI_STATUS_SUCCESS);
    }

    status = create_lag_member_full(lag_api, &global_members[15], lag_b, 0x0200000030UL);
    assert(status == SAI_STATUS_SUCCESS);
    printf("created 16 members across 2 lags\n");

    sai_object_id_t overflow_member;
    status = create_lag_member_full(lag_api, &overflow_member, lag_b, 0x0200000031UL);
    assert(status != SAI_STATUS_SUCCESS);
    printf("17th member failed \n\n");
